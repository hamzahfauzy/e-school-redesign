    ...
    <div class="row">
        <div class="col-md-12">
        	<div class="content-wrapper">
	            <h2>Tambah Pelajaran Kelas {{$classroom->name}}</h2>

	            <form method="post" action="{{route('sistem-informasi.classrooms.studies.store')}}">
	            	{{csrf_field()}}
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control z-techno-el @error('study') is-invalid @enderror" name="study">
                            ...
                            @foreach($studies as $study)
                                <option value="{{ $study->id }}" {{old('study') == $study->id ? 'selected' : ''}}>{{ $study->name }}</option>
                            @endforeach
                        </select>
                        @error('study')
